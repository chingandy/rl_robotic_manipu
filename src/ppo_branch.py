...
class CategoricalActorCriticNet(nn.Module, BaseNet):
    def __init__(self,
        ...
        hidden_actions = 128
        self.fc_action = layer_init(nn.Linear(actor_body.feature_dim, hidden_actions), 1e-3)
        self.fc_critic = layer_init(nn.Linear(critic_body.feature_dim, 1), 1e-3)
        self.ordinal_distribution = ordinal_distribution

        self.action_dim_1 = layer_init(nn.Linear(hidden_actions, action_dim), 1e-3)  # here 'action_dim' represents the bins of each dimension of action.
        self.action_dim_2 = layer_init(nn.Linear(hidden_actions, action_dim), 1e-3)  # TODO: to generalize to multiple dimentions
        self.action_range = np.linspace(-3.0, 3.0, action_dim)
        self.softmax = nn.Softmax(dim=0)

        """ the parameters for the actor and the crititic """
        self.actor_params = list(self.actor_body.parameters()) + list(self.fc_action.parameters()) + list(self.action_dim_1.parameters()) + list(self.action_dim_2.parameters())
    ...
    def forward(self, obs, action=None):
        ...
        shared_representation = self.fc_action(phi_a)
        logits_dim_1 = self.action_dim_1(shared_representation)
        logits_dim_2 = self.action_dim_2(shared_representation)
        # logits_dim_1 = self.softmax(logits_dim_1)
        # logits_dim_2 = self.softmax(logits_dim_2)
        ...
        if self.ordinal_distribution:
            ...
            dist_dim_1 = _make_ordinal(logits_dim_1)
            dist_dim_2 = _make_ordinal(logits_dim_2)

        else:
            logits_dim_1 = self.softmax(logits_dim_1)
            logits_dim_2 = self.softmax(logits_dim_2)

            dist_dim_1 = torch.distributions.Categorical(logits=logits_dim_1)
            dist_dim_2 = torch.distributions.Categorical(logits=logits_dim_2)
            # dist = torch.distributions.Categorical(logits=logits)

        if action is None:
            action_dim_1 = dist_dim_1.sample()
            action_dim_2 = dist_dim_2.sample()  # both output the order of the chosen bin

        else:
            action_dim_1 = action[0][0].item()
            action_dim_2 = action[0][1].item()
            # convert back to the order of the action bins
            # print("action_dim_1: ", action_dim_1.item())
            # print("check: ", np.where(self.action_range == action_dim_1))
            # quit()
            # print("#" * 100)
            # print(action_dim_1)
            # print(np.where(self.action_range == action_dim_1))
            # print(tensor(np.where(self.action_range == action_dim_1)))
            idx_dim_1 = np.where(self.action_range == action_dim_1)
            idx_dim_2 = np.where(self.action_range == action_dim_2)
            # print("idx_dim_1: ", idx_dim_1)
            # find the index of the nearest value
            if len(idx_dim_1[0]) == 0:
                # print("#"* 100)
                # print("idx_dim_1:", idx_dim_1[0], type(idx_dim_1[0]))
                idx_dim_1 = np.abs(np.subtract.outer(self.action_range, action_dim_1)).argmin(0)
                # print("After: ", idx_dim_1, self.action_range[idx_dim_1])
                # print("Original value: ", action_dim_1)
                # quit()

            if len(idx_dim_2[0]) == 0:
                # print("#"* 100)
                # print("idx_dim_2:", idx_dim_2[0], type(idx_dim_2[0]))
                idx_dim_2 = np.abs(np.subtract.outer(self.action_range, action_dim_2)).argmin(0)

            action_dim_1 = tensor(idx_dim_1)
            action_dim_2 = tensor(idx_dim_2)

        # log_prob

        log_prob_dim_1 = dist_dim_1.log_prob(action_dim_1)
        log_prob_dim_2 = dist_dim_2.log_prob(action_dim_2)

        # entropy
        entropy_dim_1 = dist_dim_1.entropy().unsqueeze(-1)
        entropy_dim_2 = dist_dim_2.entropy().unsqueeze(-1)

        # print("action_dim_1: ")
        # print(action_dim_1)
        # print(action_dim_1.item())
        # print(type(action_dim_1.item()))
        # print("")
        # print(self.action_range[action_dim_1])
        # print(type(self.action_range[action_dim_1]))
        # quit()
        try:
            action_dim_1 = tensor(self.action_range[int(action_dim_1.item())]).unsqueeze(0)
            action_dim_2 = tensor(self.action_range[int(action_dim_2.item())]).unsqueeze(0)
        except IndexError:
            print("IndexError .........")
            print(type(action_dim_1.item()))
            quit()
            print(self.action_range[action_dim_1.item()])
        # print("a1: ", action_dim_1)
        # print("a2: ", action_dim_2)
        action = torch.cat([action_dim_1, action_dim_2], 0)
        action = action.unsqueeze(0)
            # print("action: ", action)
            # quit()

        log_prob = log_prob_dim_1 + log_prob_dim_2
        entropy = entropy_dim_1 + entropy_dim_2
...
def ppo_feature(**kwargs):
    ...
    config.optimizer_fn = lambda params: torch.optim.Adam(params, lr=3e-4, eps=1e-5)
    config.network_fn = lambda: CategoricalActorCriticNet(config.state_dim, config.action_dim, FCBody(config.state_dim), ordinal_distribution=True)
    config.discount = 0.99
    config.use_gae = True
    config.gae_tau = 0.95
    config.entropy_weight = 0.01
    ...
    import csv
    save_dir = 'data/ppo_branch/' + args.observation + '.csv'
